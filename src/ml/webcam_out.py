...
def load_darknet_weights(model, weights_file):
    ...
    major, minor, revision, seen, _ = np.fromfile(wf, dtype=np.int32,
                                                  count=5)  # Define names of the Yolo layers (just for a reference)
    layers = ['yolo_darknet',
              'yolo_conv_0',
              'yolo_output_0',
              'yolo_conv_1',
        ...
        for i, layer in enumerate(sub_model.layers):
            ...
            if batch_norm is None:
                ...
                bn_weights = bn_weights.reshape((4, filters))[
                    [1, 0, 2, 3]]  # darknet shape (out_dim, in_dim, height, width)
...
def YoloV3(size=None, channels=3, anchors=yolo_anchors,
    masks=yolo_anchor_masks, classes=1):
...
yolo = YoloV3()
load_darknet_weights(yolo, f'{sys.argv[1]}')
